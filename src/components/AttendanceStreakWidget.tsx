...
export const AttendanceStreakWidget: React.FC = () => {
  const { streakState, claimStreakMilestone } = useApp();

  const currentDays = streakState.currentStreakDays; // 7
      ...
      <div className="pt-2 pb-1 relative z-10">
        ...
        <div className="relative flex items-center justify-between w-full px-4">
          ...
          <div className="absolute top-1/2 left-6 right-6 -translate-y-1/2 h-1 bg-[#0D1B2A] rounded-full z-0 border border-[#1F334D]">
            {/* Active filled line up to 7 days (index 1 of 3 segments = ~33%) */}
            <div
          ...
          {/* 4 Step Checkpoints */}
          {streakState.milestones.map((milestone) => {
            const isReached = currentDays >= milestone.day;
            const isClaimed = milestone.claimed;
            const isNextTarget = !isClaimed && isReached;

            return (
              <div key={milestone.day} className="flex flex-col items-center gap-1 relative z-10">
                {/* Node Circle */}
                <button
                  disabled={!isReached || isClaimed}
                  onClick={() => claimStreakMilestone(milestone.day)}
                  ...
                  title={`${milestone.day}일 마일스톤 (+${milestone.dpBonus} DP)`}
                >
                  {isClaimed ? (
